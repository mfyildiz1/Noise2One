...
def create_dataset_gopro(dataset_name,split,opt):
    data_loader = CustomDatasetDataLoader_gopro(dataset_name, split, opt)
    dataset = data_loader.load_data()
    return dataset

class CustomDatasetDataLoader_gopro():
    def __init__(self, dataset_name, split, opt):
        self.opt = opt
        dataset_class = find_dataset_using_name(opt.dataset_mode)
        self.dataset = dataset_class(opt, split)

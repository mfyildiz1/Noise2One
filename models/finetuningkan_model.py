...
def make_coord(shape, ranges=None, flatten=True):
    ...
    return ret
class FinetuningkanModel(BaseModel):
    @staticmethod
    ...
    def set_input(self, input):
        ...
        if self.eval_style.startswith('gauss'):  
            self.noise_level = self.std
        elif self.eval_style.startswith('poisson'):   
            self.noise_level = 1/self.std
    ...
    def forward_psnr(self,init):
        ...
        with torch.no_grad():
            self.zeros = torch.zeros(self.lr.shape[0],1,1,1).to(self.device,dtype = torch.float32)                               
            self.score = self.scorenet(self.lr,self.zeros)[0]  
            self.recon = self.decoder(self.lr,self.score,self.noise_level)
            b,c,h,w = self.lr.shape
    ...
    def backward_decoder(self):
        ...
        self.loss_tuning = self.criterionL1(self.recon,self.hr)
